...
A dispatcher is by default available in an action's `run()` method.
...
- `json_message`: A custom json payload as a dictionary. It can be used to send [channel specific responses](d).
...
- `template`: The name of a response template to return to the user. This response template should
              be specified in your assistants [domain](D).
...
- `**kwargs`: arbitrary keyword arguments, which can be 
              used to specify values for [variable interpolation](d) in [response templates](d). For example, 
