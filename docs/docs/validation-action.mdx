...
`ValidationAction` or `FormValidationAction` class, depending on the context in which you prefer to set or update slots.
...
Please extend the [`FormValidationAction`](./validation-action.mdx#formvalidationaction-class) class for applying
custom slot mappings only within the context of a form.
:::

### How to subclass `ValidationAction`

Firstly, you must add the name of this action to the domain `actions` list: `action_validate_slot_mappings`.
