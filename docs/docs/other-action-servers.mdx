...
When building your own custom action server, there is no requirement for 
the structure of the actions themselves, as long as the required [endpoint](api-spec.mdx) 
is implemented and the content returned by an action call is a list of [events](other-action-server-events.mdx).


To better understand the format of the information sent to and expected from
your action server by the Rasa server, consider the following example:
...
The action will set the slot, `temperature` to the maximum temperature of the weather forecast.
...
See the response sample of the [API spec](api-spec.mdx) for the expected formats. 
