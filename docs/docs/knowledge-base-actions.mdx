...
<img alt="image" src={useBaseUrl("/img/knowledge-base-example.png")} />
...
<a aria-hidden="true" tabIndex="-1" className="anchor enhancedAnchor" id="create-knowledge-base"></a>
...
(see [Creating Your Own Knowledge Base](./knowledge-base-actions.mdx#custom-knowledge-base)).
...
If it doesn't, you'll have to [customize your InMemoryKnowledgeBase](./knowledge-base-actions.mdx#customize-in-memory-knowledge-base).
...
* we will use [synonyms](https://rasa.com/docs/rasa/training-data-format.mdx#entity-synonyms) extensively
...
* `object_type`: Whenever a training example references a specific object type from your knowledge base, the object type should
  be marked as an entity. Use [synonyms](https://rasa.com/docs/rasa/training-data-format.mdx#entity-synonyms) to map e.g. `restaurants` to `restaurant`, the correct
...
* `mention`: If the user refers to an object via “the first one”, “that one”, or “it”, you should mark those terms
  ...
  in [resolving mentions](./knowledge-base-actions.mdx#resolve-mentions).
...
it is important that you use [Entity Synonyms](https://rasa.com/docs/rasa/next/training-data-format#entity-synonyms) to map “the first one” in your NLU data to “1”:
...
`KnowledgeBase` implementation (see [Customizing the InMemoryKnowledgeBase](./knowledge-base-actions.mdx#customize-in-memory-knowledge-base)).
...
<a aria-hidden="true" tabIndex="-1" className="anchor enhancedAnchor" id="customize-in-memory-knowledge-base"></a>
...
[Customizing the InMemoryKnowledgeBase](./knowledge-base-actions.mdx#customize-in-memory-knowledge-base).
