...
If you decide to use special action types, such as [slot validation actions](https://rasa.com/docs/rasa/next/slot-validation-actions),
note that you must follow the naming convention for slot validation actions: `action_validate_slot_mappings` and `validate_<form name>`.
In addition, these actions will be expected to behave like the Rasa SDK [`ValidationAction` class](./validation-action.mdx#validationaction-class-implementation)
and [`FormValidationAction` class](./validation-action.mdx#formvalidationaction-class-implementation).
