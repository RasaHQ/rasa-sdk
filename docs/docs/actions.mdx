...
the effects of an action are whatever the action
server returns when an action call is made. 
It is up to the action server to call the correct 
code according to the action name that Rasa provides.
This leads naturally to seeing actions as discrete objects
or classes, but you can implement actions however you would like.
