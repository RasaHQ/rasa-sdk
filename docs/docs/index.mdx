...
of [responses](https://rasa.com/docs/rasa/next/responses) and [events](events.mdx). The Rasa server then returns
...
the [required APIs](api-spec.mdx).  

### Rasa SDK (Python)

Rasa SDK is a Python SDK for running custom actions. It implements
the required APIs and offers methods for interacting with the conversation tracker.
...
See the [API spec](api-spec.mdx) for details about the required APIs.
