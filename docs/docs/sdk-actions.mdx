...
The `Action` class is the base class for any custom action. 
It has two methods
that must both to be overwritten, `name` and `run`. The
action server will call an action according to the return value 
of its `name` method when it receives a request to run an action from Rasa.
...
#### **Returns**
  ...
  A list of `rasa_sdk.events.Event` instances. These events are returned 
